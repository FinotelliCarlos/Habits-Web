...
export const SummaryTable = (): JSX.Element => {
  return (
    <div className="w-full flex">
      <div className="grid grid-rows-7 grid-flow-row gap-3 ">
        {WEEK_DAYS.map((weekDay, index) => {
          return (
            <div
              key={`${weekDay}-${index}`}
              className="text-zinc-400 text-xl h-1- w-10 font-bold flex items-center justify-center"
            >
              {weekDay}
            </div>
          )
      ...
      <div className="grid grid-rows-7 grid-flow-col gap-3 overflow-auto">
        ...
        {AMOUNT_OF_DAYS_TO_FILL > 0 &&
          Array.from({ length: AMOUNT_OF_DAYS_TO_FILL }).map((_, index) => {
            return (
              <div
                key={index}
